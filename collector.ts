...
export class MetadataCollector {
  ...
  public getMetadata(
    ...
    const nodeMap = new Map<MetadataValue | ClassMetadata | InterfaceMetadata | FunctionMetadata, ts.Node>();
    ...
    function errorSym(message: string, node?: ts.Node, context?: { [name: string]: string }): MetadataError {
      ...
      for (const member of classDeclaration.members) {
        ...
        switch (member.kind) {
          ...
          case ts.SyntaxKind.MethodDeclaration:
            ...
            const parameterDecoratorData: ((MetadataSymbolicExpression | MetadataError)[] | undefined)[] = [];
    ...
    const isExportedIdentifier = (identifier?: ts.Identifier) => identifier && exportMap.has(identifier.text);
    const isExported = (
      node:
          ...
          if (moduleSpecifier && moduleSpecifier.kind == ts.SyntaxKind.StringLiteral) {
            ...
            if (exportClause) {
              moduleExport.export = exportClause.elements.map(
                spec => (spec.propertyName ? { name: spec.propertyName.text, as: spec.name.text } : spec.name.text)
              );
            }
            if (!exports) exports = [];
            ...
            if (variableDeclaration.name.kind == ts.SyntaxKind.Identifier) {
              ...
              if (typeof varValue == 'string' || typeof varValue == 'number' || typeof varValue == 'boolean') {
                ...
                if (varValue && !isMetadataError(varValue)) {
                  ...
                  locals.define(
                    nameNode.text,
                    recordEntry(errorSym('Reference to a local symbol', nameNode, { name: nameNode.text }), node)
                  );
                }
              }
            } else {
  ...
  function validateFunction(functionDeclaration: FunctionMetadata) {
    if (functionDeclaration.value) {
      ...
      if (functionDeclaration.parameters) {
        ...
        if (functionDeclaration.parameters) functionDeclaration.parameters.forEach(n => locals.add(n));
      }
    ...
    try {
      ...
      if (shouldReportNode(node)) {
        if (node) {
          ...
          throw new Error(
            `${sourceFile.fileName}:${line + 1}:${character +
              1}: Error encountered in metadata generated for exported symbol '${name}': \n ${e.message}`
          );
        }
