...
export class Symbols {
  private _symbols: Map<string, MetadataValue> | undefined = undefined;
  ...
  private buildImports(): void {
    ...
    if (!symbols) {
      throw new Error('The symbol table is not initialized');
    }
      ...
      switch (node.kind) {
        case ts.SyntaxKind.ImportEqualsDeclaration:
          ...
          if (importEqualsDeclaration.moduleReference.kind === ts.SyntaxKind.ExternalModuleReference) {
            const externalReference = <ts.ExternalModuleReference>importEqualsDeclaration.moduleReference;
          ...
          if (bindings) {
            switch (bindings.kind) {
              ...
              case ts.SyntaxKind.NamespaceImport:
                ...
                symbols.set((<ts.NamespaceImport>bindings).name.text, { __symbolic: 'reference', module: from });
                break;
            }
          }
          break;
