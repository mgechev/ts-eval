...
export function recordMapEntry<T extends MetadataEntry>(
  ...
  if (!nodeMap.has(entry)) {
    ...
    if (
      node &&
      (isMetadataImportedSymbolReferenceExpression(entry) || isMetadataImportDefaultReference(entry)) &&
      entry.line == null
    ) {
  ...
  public evaluateNode(node: ts.Node, preferReference?: boolean): MetadataValue {
    ...
    switch (node.kind) {
      ...
      case ts.SyntaxKind.TypeReference:
        ...
        const getReference: (typeNameNode: ts.Identifier | ts.QualifiedName) => MetadataValue = node => {
      ...
      case ts.SyntaxKind.UnionType:
        ...
        const references = unionType.types
          .filter(n => n.kind != ts.SyntaxKind.NullKeyword && n.kind != ts.SyntaxKind.UndefinedKeyword)
      ...
      case ts.SyntaxKind.TemplateExpression:
        ...
        if (this.isFoldable(node)) {
          return templateExpression.templateSpans.reduce(
            (previous, current) =>
              previous + <string>this.evaluateNode(current.expression) + <string>this.evaluateNode(current.literal),
